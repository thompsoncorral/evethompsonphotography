...
export async function onRequestGet({ env, request, waitUntil }) {
  ...
  // Temporary: ?debug=1 attaches per-product diagnostics (catalog lookup
  // status, raw description length) to help track down why a description
  // occasionally comes back empty. Also implies refresh=1's cache bypass,
  // since a cached response has none of this info attached.
  ...
  try {
    ...
    const detailed = await Promise.all(
      summaries.map(async (summary) => {
        ...
        let debugInfo = null;
        if (debug) {
          debugInfo = { firstCatalogVariantId, syncVariantCount: (detailData.result.sync_variants || []).length };
        }
        if (firstCatalogVariantId) {
          try {
            ...
            if (debug) debugInfo.catalogStatus = catalogRes.status;
            if (catalogRes.ok) {
              const catalogData = await catalogRes.json();
              description = stripSourcingLine(catalogData.result?.product?.description || null);
              if (debug) debugInfo.rawDescriptionLength = (catalogData.result?.product?.description || "").length;
            } else if (debug) {
              debugInfo.catalogBody = (await catalogRes.text()).slice(0, 300);
            }
          } catch (err) {
            if (debug) debugInfo.error = err.message;
    ...
    // Debug responses carry extra per-product diagnostics that regular
    // shoppers shouldn't see, so never let one land in the shared cache.
