...
export async function onRequestGet({ env, request }) {
        // Temporary diagnostic switch: /api/products?debug=1 attaches why a
  // product's description came back empty (bad/missing catalog variant id,
  // a non-OK response from Printful, or a genuinely empty description on
  // Printful's side). Normal shoppers never see this -- only useful while
  // tracking down a specific product's missing "Product details" section.
                                    ...
                                    if (firstCatalogVariantId) {
                                                    try {
                                                                      ...
                                                                      if (catalogRes.ok) {
                                                                                          ...
                                                                                          const rawDescription = catalogData.result?.product?.description || null;
                                                                                          description = stripSourcingLine(rawDescription);
                                                                                          if (debug) {
                                                                                                                debugInfo = { firstCatalogVariantId, catalogStatus: catalogRes.status, rawDescriptionLength: rawDescription ? rawDescription.length : 0 };
                                                                                                }
                                                                      } else if (debug) {
                                                                                          debugInfo = { firstCatalogVariantId, catalogStatus: catalogRes.status, error: await catalogRes.text() };
                                                                      }
                                                    } catch (e) {
                                                                      // swallow -- description is a nice-to-have, not worth failing the product for
                                                      if (debug) debugInfo = { firstCatalogVariantId, error: e.message };
                                                    }
                                    } else if (debug) {
                                                    debugInfo = { firstCatalogVariantId: null, note: "no sync variants / no variant_id on first sync variant" };
                                    }
